      ...
      <section id="features" className="py-16 md:py-24 relative z-10 px-4 md:px-6 bg-[#0f1115]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {[
              { icon: FileText, title: "Case Intake", desc: "Upload a PDF or paste your case brief. CaseEdge extracts the key information in seconds.", tag: "First 5 minutes" },
              { icon: Network, title: "MECE Issue Tree", desc: "Break the core problem into a visual mind map of sub-issues — assign branches to team members instantly.", tag: "Minutes 5–15" },
              { icon: Grid, title: "Framework Recommender", desc: "Get 3 tailored frameworks with diagnostic questions specific to your case type.", tag: "Minutes 5–15" },
              { icon: CheckCircle, title: "Recommendation Drafter", desc: "Turn a rough idea into a structured SCR narrative — Situation, Complication, Resolution.", tag: "Minutes 15–22" },
              { icon: ShieldCheck, title: "Assumption Tracker", desc: "Log every assumption your recommendation makes. Know your weak points before judges find them.", tag: "Minutes 15–22" },
              { icon: Calculator, title: "Quantification Assistant", desc: "Build a back-of-envelope financial estimate to back your recommendation with real numbers.", tag: "Minutes 20–25" },
              { icon: HelpCircle, title: "Judge Q&A Simulator", desc: "Get 5 tough judge questions with model answers — based on your actual recommendation.", tag: "Final 15 minutes" },
              { icon: AlertTriangle, title: "Panic Mode", desc: "One button. Emergency exec summary, deck outline, and anchor argument generated instantly.", tag: "When time runs out" }
            ].map((f, i) => (
      ...
      <section id="how-it-works" className="py-16 md:py-24 relative z-10 px-4 md:px-6 max-w-7xl mx-auto overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-6 md:gap-4 relative z-10 px-2 sm:px-0">
            {[
              "Upload your case brief",
              "CaseEdge analyzes & frames the problem",
              "Build analysis using AI frameworks",
              "Draft & stress-test recommendation",
              "Walk in ready with Q&A prep & story"
            ].map((step, i) => (
      ...
      <section className="py-16 md:py-24 relative z-10 px-4 md:px-6 bg-[#0f1115]">
        <div className="max-w-4xl mx-auto">
          ...
          <FadeInView className="overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0">
            <table className="w-full min-w-[500px] border-collapse bg-[#15181e] rounded-sm overflow-hidden">
              <thead>
                <tr>
                  <th className="py-4 md:py-5 px-4 md:px-6 text-left border-b border-white/10 font-mono text-xs md:text-sm text-white/50 w-1/2">
                    What you need
                  </th>
                  <th className="py-4 md:py-5 px-4 md:px-6 text-center border-b border-white/10 font-mono text-xs md:text-sm text-white/50 w-1/4">
                    ChatGPT<br className="sm:hidden"/>/ Claude
                  </th>
                  <th className="py-4 md:py-5 px-4 md:px-6 text-center border-b-2 border-[#00d4ff] bg-[#00d4ff]/10 font-mono text-xs md:text-sm text-[#00d4ff] font-bold w-1/4">
                    CaseEdge
                  </th>
                </tr>
              </thead>
              <tbody>
                {[
                  "Case-specific framework recommendations",
                  "MECE issue tree visualization",
                  "SCR recommendation structure",
                  "Assumption extraction",
                  "Judge Q&A simulation",
                  "Competition timer with phase nudges",
                  "Session export as PDF",
                  "Built for MBA case competitions"
                ].map((row, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="py-3 md:py-4 px-4 md:px-6 text-xs md:text-sm text-white/80 font-medium">{row}</td>
                    <td className="py-3 md:py-4 px-4 md:px-6 text-center text-sm font-bold text-red-500/80">✗</td>
                    <td className="py-3 md:py-4 px-4 md:px-6 text-center text-sm font-bold text-[#00d4ff] bg-[#00d4ff]/5">✓</td>
                  </tr>
                ))}
              </tbody>
            </table>
